...
/**
 * An {@code OperationBatcher} is a highly-efficient data processor that is backed by an <a
...
public class OperationBatcher<T> implements Supplier<Operation<T>> {
	...
	public OperationBatcher(@Nonnull final Supplier<T> dataSupplier,
	 ...
	 * @return {@literal this}
	 */
	public OperationBatcher<T> commit(List<Operation<T>> ops) {
	 ...
	 * @return {@literal this}
	 */
	public OperationBatcher<T> batch(int size, Consumer<T> mutator) {
		long start = -1;
		long end = 0;
