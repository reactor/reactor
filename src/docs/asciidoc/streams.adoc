...
h|Streams.combineLatest(*Publisher<T1>, Publisher<T2>*, _Publisher<T3-N>\*_, *Function<Tuple2-N, C>*)
|C
...
h|Streams.join(*Publisher<T>, Publisher<T>*, _Publisher<T>*>_)
|List<T>
...
h|Promises.<T>prepare(*Environment, Dispatcher*), Promises.ready()
|T
|T
...
h|subscribe(*Subscriber<T>*), +subscribeOn
|void
...
h|consume(_Consumer<T>,Consumer<T>,Consumer<T>_), _consumeOn_
|Control
...
h|batchConsume(*Consumer<T>*, _Consumer<T>, Consumer<T>_, *Function<Long,Long>*) _batchConsumeOn_
|Control
...
h|adaptiveConsume(*Consumer<T>*, _Consumer<T>, Consumer<T>_, *Function<Stream<Long>,Publisher<Long>>*), _adaptiveConsumeOn_
|Control
...
h|broadcast(), broadcastOn(Environment, Dispatcher)
|Stream<T>
...
h|Stream.next() + Promise.await(), Promise.get()...
...
h|Stream.toList() + Promise.await(), Promise.get()...
...
They also serve the purpose of <<streams.adoc#stream-flatmap,asynchronous transformations>>, since the resulting sequences will be merged.
...
h|Streams.concat(*Publisher<T>,Publisher<T>?* x7),
Streams.concat(Publisher<Publisher<T>>),
...
h|Streams.merge, Stream.mergeWith
^.^|

h|Streams.combineLatest
^.^|

h|Streams.zip, Stream.zipWith,
^.^|

h|Streams.join, Stream.joinWith
^.^|
...
h|Stream.subscribeOn(), Stream.dispatchOn()
^.^|
