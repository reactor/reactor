...
.Creating undetermined Streams with Broadcaster and Promises
...
.Terminating a Stream pipeline
[source, java]
----
Streams<A> stream = Streams.just(a);
Streams<B> transformedStream = stream.map(transformationToB); // has not yet subscribed

Assert.isTrue(transformationStream != stream);
stream.subscribe(subscriber1); //a unique unicasted pipeline (which is just+subscriber in that case) will play A to subscriber1

Control c = transformedStream.consume(consumer1); //a unique unicasted chain of Stream is created for this consumer1, replay A
//...
c.cancel(); //force this consumer to stop receiving data
----

.Operations considered terminal and explicitely subscribing
...
|Similar to broadcast() but allows for custom `Dispatcher` to be added in the mix, eventually dispatching asynchronously if the dispatcher allows so.
...
TODO The story behind Stream/Promise threading.
