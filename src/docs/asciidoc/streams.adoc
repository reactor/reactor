...
TODO Using a `Stream` and Reactive Streams standard to control, back-off or overflow massive data throughput.
...
In addition to implement directly the Reactive Streams, some `Stream` methods differ or are simply not documented by Reactive Extensions.
