...
[source,java]
----
final Streams<String> stream = Streams.createWith(
  (demand, sub) -> { // <1>
...
.Evolving to reactive microservices, part 2, parallel requests and resiliency
[cols="1,1"]
|===
^.^h|The Not So Much Win
^.^h|The Win 
...
while(tries < 3){
  try{
    ...
    return rickFriends.get(3, TimeUnit.SECONDS)
      .addAll(
