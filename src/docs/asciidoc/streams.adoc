...
** Unlike <<core-processor#core-processor,Core Processors>>, they will usually not bother about buffering data if there is no subscriber attached
...
[source,java]
----
final Stream<String> stream1 = Streams.create(sub -> {
...
* Using `PublisherFactory.create(args)` or `Streams.createWith(args)` to just use Functional consumers for every lifecycle step (requested, stopped, started).
...
<3> Executed once on start, this is also where we initialize the shared context optionnally, every request callbacks will receive 0 from `context()`
<4> Executed once on any Termination event : *cancel()*, *onComplete()* or *onError(e)*.
...
* *Broadcaster might anyway be promoted to Processor in 2.1+ achieving the same thing and removing the need for the Reactor user to struggle picking between Processor and Broadcaster*.
****
====
...
.Output
----
03:14:42.013 [main] INFO  elapsed - subscribe: ScanAction-{{dispatcher=immediate, max-capacity=infinite}}
...
03:14:42.519 [hash-wheel-timer-run-3] INFO  elapsed - complete: SortAction-{{dispatcher=immediate, max-capacity=2147483647}{push} wrapped={push}}{ batchSize=0/2147483647 [0%]
03:14:42.520 [hash-wheel-timer-run-3] INFO  elapsed - cancel: SortAction-{{dispatcher=immediate, max-capacity=2147483647}{push} wrapped={push}}{ batchSize=0/2147483647 [0%]
----
