...
A `Publisher` like a `Stream` or a `Promise` are ideal to confront `MicroService` latency and failover. To improve the situation with better error isolation and non-blocking service calls, code has to be designed with these two constraints in mind. Part of the migration story to reactive architecture is to work step by step with quick wins with few adjustements, test and iterate to the next step.
...
.Evolving to reactive microservices, part 1, error isolation and non-blocking
[cols="1,1"]
|===
...
public class UserService{
  ...
  public Future<List<User>> all() throws Exception {    
    final CompletableFuture<Result> f = 
      new CompletableFuture<Result>();

    userDB.findAll().onResult( r ->
      f.complete(convertToList(r));
    );
    return f;
  }
...
public class UserService{
  ...
  public Stream<User> all()  {    
    return Streams
      .defer(() -> Streams.from(userDB.findAll()))
      .map(this::convert)
      .dispatchOn(cachedDispatcher());
      .subscribeOn(workDispatcher());
  }
