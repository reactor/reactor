...
Streams<Long> actionChain1 = stream.map(String::toUpperCase).filter(w -> w.equals("c")); // has not yet subscribed
Streams<Long> actionChain2 = stream.dispatchOn(sharedDispatcher()).take(5).count(); // has not yet subscribed

actionChain1.consume(System.out::println); //a unique unicasted pipeline (which is just+subscriber in that case) will play A to subscriber1
Control c = actionChain2.consume(System.out::println); //a unique unicasted chain of Stream is created for this consumer1, replay A
//...
