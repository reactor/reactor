...
One common purpose for *Reactive Streams* and *Reactive Extensions* is to be non-opinionated on the thread thanks to the signal callbacks.
...
However signals and requests can run on 2 asymmetric threads which `Actions` take care of.
