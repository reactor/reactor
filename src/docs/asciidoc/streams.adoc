...
For *Fan-Out* between subscribers from a unified pipeline, `Stream.process(Processor)`, `Stream.broadcast()`, `Stream.broadcastOn()` and `Stream.broadcastTo()` can be used.
...
Stream<String> sharedStream = actionChain1.broadcast();
