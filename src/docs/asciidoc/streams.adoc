...
[IMPORTANT]
Hot Data Sources will never be replayed (with an exception for `BehaviorBroadcaster` last emitted element). Subscribers will only see data from the moment T they have been passed to `Stream.subscribe(Subscriber)`.
...
|Emit a single `onNext(0L)` then `onComplete()` once the delay is elapsed. Be sure to pass the optional argument `Timer` if there is no current active `Environment`.

|Streams.period(period, unit, timer?)
|Emit  of N `onNext(Long)`
