...
public final class ObjectUtilsTests extends TestCase {
	...
	public void testNullSafeToStringWithPlainOldString() {
		assertEquals("I shoh love tha taste of mangoes", ObjectUtils.nullSafeToString("I shoh love tha taste of " +
				"mangoes"));
	}
	...
	public void testContainsConstant() {
		assertThat(ObjectUtils.containsConstant(Tropes.values(), "FOO"), CoreMatchers.is(true));
		assertThat(ObjectUtils.containsConstant(Tropes.values(), "foo"), CoreMatchers.is(true));
		assertThat(ObjectUtils.containsConstant(Tropes.values(), "BaR"), CoreMatchers.is(true));
		assertThat(ObjectUtils.containsConstant(Tropes.values(), "bar"), CoreMatchers.is(true));
		assertThat(ObjectUtils.containsConstant(Tropes.values(), "BAZ"), CoreMatchers.is(true));
		assertThat(ObjectUtils.containsConstant(Tropes.values(), "baz"), CoreMatchers.is(true));

		assertThat(ObjectUtils.containsConstant(Tropes.values(), "BOGUS"), CoreMatchers.is(false));

		assertThat(ObjectUtils.containsConstant(Tropes.values(), "FOO", true), CoreMatchers.is(true));
		assertThat(ObjectUtils.containsConstant(Tropes.values(), "foo", true), CoreMatchers.is(false));
	}

	public void testCaseInsensitiveValueOf() {
		assertThat(ObjectUtils.caseInsensitiveValueOf(Tropes.values(), "foo"), CoreMatchers.is(Tropes.FOO));
		assertThat(ObjectUtils.caseInsensitiveValueOf(Tropes.values(), "BAR"), CoreMatchers.is(Tropes.BAR));
		try {
			ObjectUtils.caseInsensitiveValueOf(Tropes.values(), "bogus");
			fail("expected IllegalArgumentException");
		} catch (IllegalArgumentException ex) {
			assertThat(ex.getMessage(),
					CoreMatchers.is("constant [bogus] does not exist in enum type " +
							"reactor.core.support.ObjectUtilsTests$Tropes"));
		}
	}
