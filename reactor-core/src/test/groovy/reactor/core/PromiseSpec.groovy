...
class PromiseSpec extends Specification {
	...
	def "Promise error handling"() {
		...
		def promise = R.promise(supplier { throw new Exception('bad') }).sync().get()
	...
	def "Test promise chaining with exception"() {
		...
		def promise = R.promise(supplier { 1 + 1 }).sync().get()
	...
	def "Test promise list handling"() {
		...
		def latch = new CountDownLatch(1)
		R.<Integer>promise().merge([p1, p2]).get().onSuccess consumer { List<Integer> v -> result = v; latch.countDown(); }
	...
	def "Test promise list with an exception"() {
		...
		def p1 = R.promise supplier { 1 + 1 } get()
		def p2 = R.promise supplier { throw new Exception('bad') } get()
		def p3 = R.promise supplier { 2 + 2 } get()
