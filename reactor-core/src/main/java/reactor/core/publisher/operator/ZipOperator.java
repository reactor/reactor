...
public final class ZipOperator<TUPLE extends Tuple, V>
		...
		void drainLoop(ZipState[] inner) {
			...
			for (; ; ) {
				...
				for (; ; ) {
					...
					final Object[] tuple = new Object[n];
	...
	static final class SingleBufferSubscriber<V> extends BaseSubscriber<Object> implements Bounded {
		...
		SingleBufferSubscriber(ZipBarrier<?, V> parent) {
			this.parent = parent;
		}

		@Override
		public void onSubscribe(Subscription subscription) {
			long r = parent.getRequested();
			if (r != 0) {
