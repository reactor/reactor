...
public class IOStreams extends Streams {
	 ...
	 * Implementing the MapStream contract means that subscribers will have the ability to listen for put, putAll,
	 * clear
	 * and delete operations.
	 * <p>
	 * A persistent map is read/write capable and will use an internal cached map for read operations. This cache
	 * will be
	 * updated when an operation has been notified. Read-only persistent map can be created with {@link
	 * #persistentMapReader(String)}.
	 * <p>
	 ...
	 *
	 * @param name         The identified name for the data store
	 * @param deleteOnExit Make a persistent store transient, useful for producer queues coupled with persistent map
	 *                        readers.
	 * @param <KEY>        the type of the key used to identify stored values
	 * @param <VALUE>      the type of the payload stored
	 * @return a new MapStream that can be notified on any operation (put, putAll...)
	 ...
	 * and delete operations.
	 * <p>
	 * A persistent map reader is read capable only and will use an internal cached map for read/write operations.
	 * This cache will be
	 * updated when an operation has been notified. Read/Write persistent map can be created with {@link
