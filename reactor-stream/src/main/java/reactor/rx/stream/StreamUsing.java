...
/**
 * Uses a resource, generated by a supplier for each individual Subscriber, while streaming the values from a Publisher
 * derived from the same resource and makes sure the resource is released if the sequence terminates or the Subscriber
 * cancels.
 * <p>
 * <p>
 * Eager resource cleanup happens just before the source termination and exceptions raised by the cleanup Consumer may
 * override the terminal even. Non-eager cleanup will drop any exception.
 *
 ...
 * @since 2.5
 */
public final class StreamUsing<T, S> extends reactor.rx.Stream<T>
	...
	static final class StreamUsingSubscriber<T, S> implements Subscriber<T>, Subscription {
		...
		public StreamUsingSubscriber(Subscriber<? super T> actual,
				Consumer<? super S> resourceCleanup,
				S resource,
				boolean eager) {
			this.actual = actual;
