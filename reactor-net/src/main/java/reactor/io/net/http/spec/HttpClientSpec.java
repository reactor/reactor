...
package reactor.io.net.http.spec;
 ...
 */
public class HttpClientSpec<IN, OUT> extends DispatcherComponentSpec<HttpClientSpec<IN, OUT>, HttpClient<IN, OUT>> {

	private final Constructor<HttpClient<IN, OUT>> clientImplConstructor;
	...
	public HttpClientSpec(@Nonnull Class<? extends HttpClient> clientImpl) {
		Assert.notNull(clientImpl, "TcpClient implementation class cannot be null.");
		try {
			this.clientImplConstructor = (Constructor<HttpClient<IN, OUT>>) clientImpl.getDeclaredConstructor(
					Environment.class,
					Dispatcher.class,
