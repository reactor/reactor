...
We can use `Flux`, or <<streams.adoc#streams-basics, Streams factories>> like `Streams.createWith()` to quickly achieve common use cases:
...
[source,java]
----
final Processor<Integer, Integer> keyboardStream = ProcessorTopic.create();
