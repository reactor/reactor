...
[source,java]
----
Processor<String,String> processor = ProcessorTopic.create();
...
* ProcessorWorkQueue supports scaling up with the number of attached subscribers.
...
* Attaching a `Processor` via `Stream.process` for instance can affect the thread too. The `Processor` such as `ProcessorTopic` will run the `Subscribers` on its managed threads.
** `request` and `cancel` will run on the processor as well if in its context already.
** `ProcessorWorkQueue` will only dispatch *onNext* signals to one `Subscriber` at most unless it has canceled in-flight (replay to a new Subscriber).
****
...
Streams
  .range(1, 100)
  .process(ProcessorTopic.create()) // <2>
  .subscribeOn(Environment.workDispatcher()) // <1>
  .capacity(1); // <3>
  .consume(); // <4>
----
...
|T
2+|Since a ProcessorWorkQueue distributes the signals to each subscribe, it is an efficient alternative to `partition()` when it's just about scaling-up, not routing.
